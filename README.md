...
公開URLの末尾に `?farm=1` を付けるとファームモードになります。初期表示は西地区で、画面左の地区選択から中地区・東地区へ切り替えられます。
...
- 残り試合に対するチーム別の想定勝率編集
- 対象球団の残り日程表示
- 残り試合の結果をカードごとに入力するマジック・優勝条件確認
- 入力結果を反映した相手別条件表と日付別の点灯・優勝判定
- 日付×球団の表で○・●・△を入力し、リーグ内対戦の相手側結果を自動反映
- ライト / ダーク表示切り替え
